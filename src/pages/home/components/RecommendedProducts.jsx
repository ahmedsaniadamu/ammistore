...
const RecommendedProducts = () => {
    ...
    const fetcher = async () => {
        const res = await fetch(process.env.REACT_APP_SERVER_URL + 'api/product.php?filter=recommended&limit=8')
        const data = await res.json() 
        return data ;
    }     
   
  const {  data } = useSWR( 'recommended-products', fetcher )
  ...
  return (
      <div className='recommended-product-wrapper d-block mt-sm-3'>         
         {             
           data.map( product => {
